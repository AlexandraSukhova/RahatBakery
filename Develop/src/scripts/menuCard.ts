...
export function createMenuPositionCard(
  cardInfo: MenuItem,
  blockName: string,
  ...
  const cardPrice = cardItem.querySelector(
    `.${blockName}-price`,
  ) as HTMLParagraphElement;
  const cardDescription = cardItem.querySelector(
    `.${blockName}-description`,
  ) as HTMLParagraphElement;
  const cardFillingList = cardItem.querySelector(`.${blockName}-filling-list`);
  const cardFillingTitle = cardItem.querySelector(
    `.${blockName}-filling-title`,
  ) as HTMLHeadingElement;
